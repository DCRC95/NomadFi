    ...
    <div className="bg-white rounded-lg shadow p-6 border">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <img 
            src={getTokenIcon(getTokenSymbol(info.name))} 
            alt={`${getTokenSymbol(info.name)} icon`}
  ...
  return (
    <div className={`bg-white rounded-lg shadow p-6 border`}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          {(() => {
            const tokenSymbol = getTokenSymbol(strategy.name, strategy.tokenAddress, strategy.chainId?.toString());
            const iconPath = getTokenIcon(tokenSymbol);
            console.log('StrategyList rendering icon:', { strategyName: strategy.name, tokenSymbol, iconPath });
            return (
              <div className="flex items-center gap-2">
                <div 
                  style={{ 
          ...
          <div>
            {(() => {
              ...
              return (
                <div>
                  <div className="text-lg font-semibold">{displayName.mainTitle}</div>
                  <div className="text-xs text-gray-500">{displayName.subtitle}</div>
                </div>
              );
            })()}
          </div>
